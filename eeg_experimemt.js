...
let expName = 'eeg_experimemt';  // from the Builder filename that created this script
let expInfo = {
...
import {Titta, helpers_tobii as helpers} from 'titta';
...
async function experimentInit() {
  ...
  thank_you_message = new visual.TextStim({
    win: psychoJS.window,
...
function welcomeRoutineBegin(snapshot) {
  return async function () {
    ...
    welcomeComponents.push(welcome_message);
...
function ThankyouRoutineBegin(snapshot) {
  return async function () {
    ...
    ThankyouComponents.push(thank_you_message);
...
function ThankyouRoutineEachFrame() {
  return async function () {
    ...
    if (t >= 0 && thank_you_message.status === PsychoJS.Status.NOT_STARTED) {
      // keep track of start time/frame for later
      thank_you_message.tStart = t;  // (not accounting for frame time here)
...
async function quitPsychoJS(message, isCompleted) {
  ...
  ttl.send_message(ttl.external_presenter_address, {"operation": "StopRecording"});
